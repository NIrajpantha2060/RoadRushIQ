...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ── NEW: power-up state flags ──────────────────
    this.shieldActive      = false;
    this._shieldSpeedBoost = 0;
    // ──────────────────────────────────────────────
    ...
    // ── NEW: Power-up system ───────────────────────
    const bikeId     = this.registry.get('selectedBike') ?? 'skooter';
  ...
  _countDangerLanes() {
    const dangerTop = this.bike.y - 600;
    const dangerBot = this.bike.y + 60;
  ...
  _checkCollisions() {
    ...
    // ── NEW: skip traffic collision while shield is active ─────────────────
  ...
  _handleInput(dt) {
    ...
    const wasAccel = up;
    const wasDecel = down || (!up && this.speed > BASE_SPEED * 0.3);
    ...
    // ── NEW: SPACE -> try to activate power-up (edge-triggered) ───────────
    const spaceDown = this.keys.space.isDown;
    if (spaceDown && !this._prevSpace) {
      this._powerUp.tryActivate();
    }
