// /**
//  * GameScene.js — Top-down 4-lane Traffic Rider style
//  *
//  * Layout:
//  *   Lane 0 | Lane 1 || Lane 2 | Lane 3
//  *   ← oncoming ←    || ← traffic ←
//  *
//  * Player rides a bike in lanes 2 or 3.
//  * Traffic in lanes 0 & 1 moves downward fast.
//  * Traffic in lanes 2 & 3 also moves downward, but slower.
...
// // ── Road layout ────────────────────────────────
// const LANES        = 4;
// const ROAD_FRAC    = 0.80;   // road takes 80% of screen width
...
// // ── Spawn ──────────────────────────────────────
// const ONCOMING_SPEED_BONUS = 120; // added to player speed for oncoming feel
...
// 		this.playerZ     = 0;
// 		this.currentLane = 2;   // player starts in lane 2
// 		this.fromLane    = 2;
// 		this.toLane      = 2;
// 		this.laneT       = 1;
...
// 			this.bikeShadow.x   = x;
// 			this.bikeGlow.x     = x;
// 			this.bike.angle     = (this.toLane > this.fromLane ? 1 : -1)
// 				* 12 * Math.sin(this.laneT * Math.PI);
// 			if (this.laneT >= 1) {
// 				this.isChanging     = false;
...
// 		// Registry speed display (km/h feel)
...
// 		// Road surface — left half (oncoming, slightly different shade)
// 		const halfW = this.roadWidth / 2;
// 		this.add.rectangle(
// 			this.roadLeft + halfW / 2, H / 2, halfW, H, 0x1a1c30
// 		).setDepth(1);
// 		// Road surface — right half (player lanes)
// 		this.add.rectangle(
// 			this.roadLeft + halfW + halfW / 2, H / 2, halfW, H, 0x1e2038
// 		).setDepth(1);
...
// 	_createDivider() {
// 		const H = this.H;
// 		// Bold yellow centre divider between lane 1 and lane 2
...
// 		// Dashes between lane 0-1 (oncoming side) and between lane 2-3 (player side)
// 		const dashCols = [1, 3]; // column positions (between lanes)
...
// 				dash.y -= (Math.ceil(H / this._dashCycle) + 2) * this._dashCycle;
// 			}
// 		}
// 	}
...
// 		// Speed line graphics (redrawn each frame)
...
// 	_spawnTraffic() {
// 		const isOncoming = Math.random() < 0.52;
// 		const lanePool   = isOncoming ? [0, 1] : [2, 3];
// 		const lane       = Phaser.Utils.Array.GetRandom(lanePool);
// 		const isTruck    = Math.random() < 0.30;

// 		const COLORS = [0xe74c3c, 0x2ecc71, 0xf39c12, 0x9b59b6, 0xdddddd, 0x1abc9c, 0xe67e22, 0x3498db, 0xfd79a8];
// 		const color  = Phaser.Utils.Array.GetRandom(COLORS);

// 		const spawnY = isOncoming
// 			? -(isTruck ? 60 : 50)
// 			: this.H + (isTruck ? 60 : 50);

// 		const gfx = isTruck
// 			? this._drawTruck(color)
// 			: this._drawCar(color);
...
// 			speed: isOncoming
// 				? this.speed * 0.7 + ONCOMING_SPEED_BONUS
// 				: this.speed * 0.48,
// 		});
// 	}
...
// 			// Recalculate speed each frame so it adapts
// 			tr.speed = tr.oncoming
// 				? this.speed * 0.7 + ONCOMING_SPEED_BONUS
// 				: this.speed * 0.48;

// 			const dy = tr.oncoming ? tr.speed * dt : -tr.speed * dt;
// 			tr.container.y += dy;
// 			tr.shadow.y    += dy;
// 		}
...
// 			const offscreen = tr.oncoming ? y > this.H + 100 : y < -100;
// 			if (offscreen) {
 ...
 *   ↑ oncoming ↑    || ↓ same-dir ↓
 *
 * Player rides in lanes 2 or 3 (right side).
 * Left lanes (0,1): oncoming traffic moves UPWARD (bottom → top).
 * Right lanes (2,3): same-direction traffic moves DOWNWARD (top → bottom),
 *   but slower than the player's perceived speed, so they overtake from behind.
 *
 * Up/Down = accelerate / brake
 * Left/Right = lane change
 */
...
export default class GameScene extends Phaser.Scene {
  ...
  _drawBike() {
    ...
    // Rear wheel (bottom of screen = behind player)
    g.fillStyle(0x111122, 1);
    g.fillEllipse(0, 22, 18, 10, 16);
    g.fillStyle(0x3a3a5c, 1);
    g.fillEllipse(0, 22, 12, 7, 16);
    ...
    // Front wheel (top of screen = ahead)
    g.fillStyle(0x111122, 1);
    g.fillEllipse(0, -22, 16, 9, 16);
    g.fillStyle(0x3a3a5c, 1);
    g.fillEllipse(0, -22, 10, 6, 16);
  ...
  /**
   * Traffic direction:
   *  - Lanes 0,1 (left/oncoming): spawn at BOTTOM, move UPWARD (negative dy)
   *    These vehicles are flipped so their front faces upward.
   *  - Lanes 2,3 (right/same-dir): spawn at TOP, move DOWNWARD (positive dy)
   *    They move faster than player sees behind, so they "come from behind".
   *    Speed = player speed * 1.35 so they overtake the player.
   */
  _spawnTraffic() {
    ...
    if (isOncoming) {
      ...
      // Comes from top, moves down (behind the player)
      spawnY = -halfH;
    }
    ...
    // Flip oncoming vehicles so their fronts face upward (toward player)
    if (isOncoming) container.scaleY = -1;
  ...
  _updateTraffic(dt) {
    for (const tr of this._traffic) {
      let dy;
      if (tr.oncoming) {
        // Moves upward — relative speed = player speed + its own speed
        const speed = this.speed * 0.65 + 100;
        dy = -speed * dt;
      } else {
        // Same direction — moves downward, but faster than player scroll
        // Player is "moving up" relative to the world; same-dir cars appear
        // to come from behind when their world-speed > player world-speed.
        // We represent it by making them scroll down slower than the road
        // but still moving — the player "catches up" visually.
        // Speed > 0 → they scroll down; player overtakes them (realistic).
        // To make them come FROM BEHIND, set speed > player speed equivalent.
        const speed = this.speed * 1.30;
        dy = speed * dt;
      }
