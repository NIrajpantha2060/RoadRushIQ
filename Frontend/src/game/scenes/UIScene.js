...
//   // ── Cleanup helper — only called when a run is truly ending ──────────────
//   _finalizeGameOver() {
//     const key = this._getGameSceneKey();
//     const gameScene = key ? this.scene.get(key) : null;
//     if (gameScene) {
//       gameScene._sfx?.destroy();
//       gameScene._powerUp?.destroy();
//     }
//   }
...
//     // Dim background
//     push(this.add.rectangle(cx, cy, W, H, 0x000000, 0.85).setDepth(50).setScrollFactor(0));

//     // Card (rounded corners via Graphics)
//     const cardGfx = this.add.graphics().setDepth(51).setScrollFactor(0);
//     cardGfx.fillStyle(0x0d1220, 1);
//     cardGfx.fillRoundedRect(left, top, cardW, cardH, 20);
//     cardGfx.lineStyle(2, 0xff4d8d, 0.55);
//     cardGfx.strokeRoundedRect(left, top, cardW, cardH, 20);
//     push(cardGfx);

//     // Top accent glow bar
//     const accentGfx = this.add.graphics().setDepth(52).setScrollFactor(0);
//     accentGfx.fillStyle(0xff4d8d, 0.9);
//     accentGfx.fillRoundedRect(cx - 34, top - 2, 68, 5, 3);
//     push(accentGfx);

//     // Countdown pill, top-right of card
//     const pillBg = this.add.graphics().setDepth(52).setScrollFactor(0);
//     pillBg.fillStyle(0xffffff, 0.08);
//     pillBg.fillRoundedRect(left + cardW - 56, top + 18, 40, 22, 11);
//     push(pillBg);

//     this._saveMeTimerText = this.add.text(left + cardW - 36, top + 29, '5s', {
//       fontSize: '11px', fontFamily: 'system-ui, sans-serif', fontStyle: 'bold',
//       color: 'rgba(255,255,255,0.6)',
//     }).setOrigin(0.5).setDepth(53).setScrollFactor(0);
//     push(this._saveMeTimerText);

//     // Heart icon (pulsing)
//     const heart = this.add.text(cx, top + 66, '💗', { fontSize: '46px' })
//       .setOrigin(0.5).setDepth(53).setScrollFactor(0);
//     push(heart);
//     this._saveMeHeartTween = this.tweens.add({
//       targets: heart, scale: 1.12, duration: 600,
//       yoyo: true, repeat: -1, ease: 'Sine.easeInOut',
//     });

//     // Title
//     push(this.add.text(cx, top + 118, 'SAVE ME!', {
//       fontSize: '25px', fontFamily: 'system-ui, sans-serif',
//       fontStyle: 'bold', color: '#ffffff',
//     }).setOrigin(0.5).setDepth(53).setScrollFactor(0));

//     // Subtitle
//     push(this.add.text(cx, top + 144, 'Continue your run right where you crashed', {
//       fontSize: '12px', fontFamily: 'system-ui, sans-serif',
//       color: 'rgba(255,255,255,0.45)',
...
//     // ── Watch Ad (primary) ──
//     const adY = top + 196;
//     const adGfx = this.add.graphics().setDepth(52).setScrollFactor(0);
//     const drawAdBtn = (color) => {
//       adGfx.clear();
//       adGfx.fillStyle(color, 1);
//       adGfx.fillRoundedRect(cx - btnW / 2, adY - 25, btnW, 50, 14);
//     };
//     drawAdBtn(0x00c3ff);
//     push(adGfx);

//     push(this.add.text(cx - 12, adY, '▶', {
//       fontSize: '15px', fontFamily: 'system-ui, sans-serif', fontStyle: 'bold', color: '#07090f',
//     }).setOrigin(0.5).setDepth(53).setScrollFactor(0));

//     push(this.add.text(cx + 10, adY, 'Watch Ad to Revive', {
//       fontSize: '15px', fontFamily: 'system-ui, sans-serif', fontStyle: 'bold', color: '#07090f',
//     }).setOrigin(0, 0.5).setDepth(53).setScrollFactor(0));

//     const adHit = this.add.rectangle(cx, adY, btnW, 50, 0xffffff, 0.001)
//       .setDepth(54).setScrollFactor(0)
//       .setInteractive({ useHandCursor: true })
//       .on('pointerdown', () => this._onSaveMeAd())
//       .on('pointerover',  () => drawAdBtn(0x33d6ff))
//       .on('pointerout',   () => drawAdBtn(0x00c3ff));
//     push(adHit);
...
//     const gemGfx = this.add.graphics().setDepth(52).setScrollFactor(0);
//     const drawGemBtn = (fill, strokeColor, strokeAlpha) => {
//       gemGfx.clear();
//       gemGfx.fillStyle(fill, 1);
//       gemGfx.fillRoundedRect(cx - btnW / 2, gemY - 25, btnW, 50, 14);
//       gemGfx.lineStyle(1.5, strokeColor, strokeAlpha);
//       gemGfx.strokeRoundedRect(cx - btnW / 2, gemY - 25, btnW, 50, 14);
//     };
//     drawGemBtn(canAfford ? 0x131d35 : 0x0a0e1a, canAfford ? 0x00c3ff : 0x333355, canAfford ? 0.7 : 0.3);
//     push(gemGfx);

//     push(this.add.text(cx, gemY,
//       canAfford ? `💎 Use ${SAVE_COST_GEMS} Gems` : `💎 Need ${SAVE_COST_GEMS} Gems (have ${blue})`, {
//       fontSize: '14px', fontFamily: 'system-ui, sans-serif', fontStyle: 'bold',
//       color: canAfford ? '#00c3ff' : 'rgba(255,255,255,0.28)',
//     }).setOrigin(0.5).setDepth(53).setScrollFactor(0));

//     if (canAfford) {
//       const gemHit = this.add.rectangle(cx, gemY, btnW, 50, 0xffffff, 0.001)
//         .setDepth(54).setScrollFactor(0)
//         .setInteractive({ useHandCursor: true })
//         .on('pointerdown', () => this._onSaveMeGems())
//         .on('pointerover',  () => drawGemBtn(0x1e2e50, 0x00c3ff, 0.9))
//         .on('pointerout',   () => drawGemBtn(0x131d35, 0x00c3ff, 0.7));
//       push(gemHit);
//     }

//     // ── No thanks (tertiary) ──
//     const noY  = gemY + 48;
//     const noBtn = this.add.text(cx, noY, 'No thanks', {
//       fontSize: '12px', fontFamily: 'system-ui, sans-serif',
//       color: 'rgba(255,255,255,0.32)',
//     }).setOrigin(0.5).setDepth(53).setScrollFactor(0)
//       .setInteractive({ useHandCursor: true })
//       .on('pointerdown', () => this._onSaveMeDecline())
//       .on('pointerover',  () => noBtn.setColor('rgba(255,255,255,0.6)'))
//       .on('pointerout',   () => noBtn.setColor('rgba(255,255,255,0.32)'));
//     push(noBtn);
...
//     this._saveMeInterval = setInterval(() => {
//       this._saveMeCountdown--;
//       this._saveMeTimerText?.setText(`${Math.max(this._saveMeCountdown, 0)}s`);
//       if (this._saveMeCountdown <= 0) this._onSaveMeDecline();
//     }, 1000);
...
//     clearInterval(this._saveMeInterval);
//     this._saveMeInterval = null;
//     if (this._saveMeHeartTween) { this._saveMeHeartTween.stop(); this._saveMeHeartTween = null; }
//     if (this._saveMeItems) {
//       for (const item of this._saveMeItems) item?.destroy();
//       this._saveMeItems = null;
//     }
...
//     // Bring sound and power-up back to life
//     gameScene._sfx?.resume();
//     gameScene._powerUp?.reset();
...
export default class UIScene extends Phaser.Scene {
  ...
  _showSaveMe() {
    ...
    // Single centered text instead of two separately-positioned pieces
    push(this.add.text(cx, adY, '▶  Watch Ad to Revive', {
      fontSize: '14px', fontFamily: 'system-ui, sans-serif',
      fontStyle: 'bold', color: '#07090f',
    }).setOrigin(0.5).setDepth(53).setScrollFactor(0));
