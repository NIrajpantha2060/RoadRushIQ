...
export default function App() {
  ...
  const persistRunAndRefresh = useCallback(async (runSummary) => {
    ...
    try {
      ...
      const me = await getMe();
      setUser(me);
      // selectedBike is intentionally NOT synced here — it's client-driven,
      // and syncing it from a background refresh can overwrite a bike
      // the player just picked in the Garage while this request was in flight.
    } catch (err) {
      console.error('Failed to persist run stats:', err);
    }
  }, []);
